...
public class CrazyNoteHeadCommands implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String CommandLabel, String[] args){
		...
		if(args.length < 1) {
			...
			if(sender.hasPermission("CrazyFeet.crazyNoteheadeother")) {
				if(Bukkit.getServer().getPlayer(args[0]) != null) {
					Player targ = Bukkit.getServer().getPlayer(args[0]);
					if(chNote.contains(targ)) {
						chNote.remove(targ);
						targ.sendMessage(yellow+sender.getName()+" has disabled your CrazyNoteHead!");
						sender.sendMessage(yellow+targ.getDisplayName()+"'s CrazyNoteHead has been disabled!");
						return true;
					} else {
						chNote.add(targ);
						targ.sendMessage(yellow+sender.getName()+" has given you CrazyNoteHead!");
						sender.sendMessage(yellow+targ.getDisplayName()+" has been given CrazyNoteHead!");
						return true;
					}
				} else {
				...
				sender.sendMessage(red+"You do not have permission to toggle CrazyNoteHead on others.");
				return true;
			}
		} else if(args.length > 1) {
