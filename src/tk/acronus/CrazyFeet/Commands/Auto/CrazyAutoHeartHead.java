...
public class CrazyAutoHeartHead implements CommandExecutor {
	
	private CrazyFeet p;
	
	public CrazyAutoHeartHead(CrazyFeet instance) {
		p = instance;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String CommandLabel, String[] args){
		...
		if(args.length < 1) {
			if(sender instanceof Player) {
				...
				if(player.hasPermission("CrazyFeet.crazyHearthead.autoHearthead")) {
					if(p.getAHeartheadPlayers().contains(player.getName())) {
						p.getAHeartheadPlayers().remove(player);
						p.getAHeartheadPlayers().saveAutoHeartHeadPlayers();
						player.sendMessage(yellow+"You will no longer have "+red+"CrazyHeartHead "+yellow+"enabled when joining!");
						return true;
					} else {
						p.getAHeartheadPlayers().add(player);
						p.getAHeartheadPlayers().saveAutoHeartHeadPlayers();
						player.sendMessage(yellow+"You will now have "+red+"CrazyHeartHead "+yellow+"enabled when joining!");
						return true;
					}
				} else {
			...
			if(sender.hasPermission("CrazyFeet.crazyHearthead.autoHeartheadother")) {
				if(Bukkit.getServer().getPlayer(args[0]) != null) {
					Player targ = Bukkit.getServer().getPlayer(args[0]);
					if(p.getAHeartheadPlayers().contains(targ.getName())) {
						p.getAHeartheadPlayers().remove(targ);
						p.getAHeartheadPlayers().saveAutoHeartHeadPlayers();
