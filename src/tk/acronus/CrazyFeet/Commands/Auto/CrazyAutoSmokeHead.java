...
public class CrazyAutoSmokeHead implements CommandExecutor {
	
	private CrazyFeet p;
	
	public CrazyAutoSmokeHead(CrazyFeet instance) {
		p = instance;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String CommandLabel, String[] args){
		...
		if(args.length < 1) {
			if(sender instanceof Player) {
				...
				if(player.hasPermission("CrazyFeet.crazySmokehead.autoSmokehead")) {
					if(p.getASmokeheadPlayers().contains(player.getName())) {
						p.getASmokeheadPlayers().remove(player);
						p.getASmokeheadPlayers().saveAutoSmokeHeadPlayers();
						player.sendMessage(yellow+"You will no longer have "+red+"CrazySmokeHead "+yellow+"enabled when joining!");
						return true;
					} else {
						p.getASmokeheadPlayers().add(player);
						p.getASmokeheadPlayers().saveAutoSmokeHeadPlayers();
						player.sendMessage(yellow+"You will now have "+red+"CrazySmokeHead "+yellow+"enabled when joining!");
						return true;
					}
				} else {
			...
			if(sender.hasPermission("CrazyFeet.crazySmokehead.autoSmokeheadother")) {
				if(Bukkit.getServer().getPlayer(args[0]) != null) {
					Player targ = Bukkit.getServer().getPlayer(args[0]);
					if(p.getASmokeheadPlayers().contains(targ.getName())) {
						p.getASmokeheadPlayers().remove(targ);
						p.getASmokeheadPlayers().saveAutoSmokeHeadPlayers();
