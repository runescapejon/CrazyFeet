...
public class CrazyAutoMagicHead implements CommandExecutor {
	
	private CrazyFeet p;
	
	public CrazyAutoMagicHead(CrazyFeet instance) {
		p = instance;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String CommandLabel, String[] args){
		...
		if(args.length < 1) {
			if(sender instanceof Player) {
				...
				if(player.hasPermission("CrazyFeet.crazyMagichead.autoMagichead")) {
					if(p.getAMagicheadPlayers().contains(player.getName())) {
						p.getAMagicheadPlayers().remove(player);
						p.getAMagicheadPlayers().saveAutoMagicHeadPlayers();
						player.sendMessage(yellow+"You will no longer have "+red+"CrazyMagicHead "+yellow+"enabled when joining!");
						return true;
					} else {
						p.getAMagicheadPlayers().add(player);
						p.getAMagicheadPlayers().saveAutoMagicHeadPlayers();
						player.sendMessage(yellow+"You will now have "+red+"CrazyMagicHead "+yellow+"enabled when joining!");
						return true;
					}
				} else {
			...
			if(sender.hasPermission("CrazyFeet.crazyMagichead.autoMagicheadother")) {
				if(Bukkit.getServer().getPlayer(args[0]) != null) {
					Player targ = Bukkit.getServer().getPlayer(args[0]);
					if(p.getAMagicheadPlayers().contains(targ.getName())) {
						p.getAMagicheadPlayers().remove(targ);
						p.getAMagicheadPlayers().saveAutoMagicHeadPlayers();
