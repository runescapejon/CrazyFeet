...
public class CrazySmokeHeadCommands implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String CommandLabel, String[] args){
		...
		if(args.length < 1) {
			...
			if(sender.hasPermission("CrazyFeet.crazySmokeheadeother")) {
				if(Bukkit.getServer().getPlayer(args[0]) != null) {
					Player targ = Bukkit.getServer().getPlayer(args[0]);
					if(chSmoke.contains(targ)) {
						chSmoke.remove(targ);
						targ.sendMessage(yellow+sender.getName()+" has disabled your CrazySmokeHead!");
						sender.sendMessage(yellow+targ.getDisplayName()+"'s CrazySmokeHead has been disabled!");
						return true;
					} else {
						chSmoke.add(targ);
						targ.sendMessage(yellow+sender.getName()+" has given you CrazySmokeHead!");
						sender.sendMessage(yellow+targ.getDisplayName()+" has been given CrazySmokeHead!");
						return true;
					}
				} else {
				...
				sender.sendMessage(red+"You do not have permission to toggle CrazySmokeHead on others.");
				return true;
			}
		} else if(args.length > 1) {
